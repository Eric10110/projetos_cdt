"""
===============================================================================
IA "SKY" - Versão Unificada e Completa
(Controle Local + Google Calendar + Pesquisa Web + Comandos do Sistema)
===============================================================================
"""
...
# 3. Opcional: Verificação da biblioteca de Pesquisa Web
try:
    from googlesearch import search as google_search
    TEM_GOOGLE_SEARCH = True
except ImportError:
    TEM_GOOGLE_SEARCH = False
    print("[AVISO] 'googlesearch-python' não encontrado. Pesquisas web usarão fallback simples. (Opcional: pip install googlesearch-python)")


MODELO_AVANCADO_OLLAMA = "llama3.1:8b"
NOME_BANCO_DADOS = "historico_sky.db"
ARQUIVO_CONFIG_JSON = "config.json"
LIMITE_MENSAGENS_RECENTES = 6
SCOPES = ['https://www.googleapis.com/auth/calendar']


# =============================================================================
# MÓDULO DE AUTOMAÇÃO E FERRAMENTAS LOCAIS
# =============================================================================
...
def executar_comando_cmd(comando: str) -> str:
    """Executa um comando de terminal/prompt de comando no sistema operacional de forma segura."""
    try:
        ...
        if not saida:
            saida = "Comando executado sem retorno de texto."
        return f"Resultado da execução:\n{saida[:1000]}"
    except subprocess.TimeoutExpired:
        return "Erro: A execução do comando excedeu o tempo limite de 15 segundos."
    except Exception as e:
        return f"Erro ao executar comando: {str(e)}"


def pesquisar_na_web(termo_busca: str, num_resultados: int = 3) -> str:
    """Realiza uma pesquisa na web para obter informações recentes ou externas."""
    try:
        if TEM_GOOGLE_SEARCH:
            resultados = []
            for url in google_search(termo_busca, num_results=num_resultados, lang="pt"):
                resultados.append(url)
            if resultados:
                return "Links encontrados para a pesquisa:\n" + "\n".join(resultados)
            return "Nenhum resultado encontrado para a busca."
        else:
            return f"Pesquisa por '{termo_busca}' registrada (Instale 'googlesearch-python' para obter URLs completas)."
    except Exception as e:
...
def agendar_compromisso_google(titulo: str, data_inicio_iso: str, duracao_minutos: int = 60) -> str:
    ...
    data_inicio_iso formato esperado: 'YYYY-MM-DDTHH:MM:SS'
    """
    try:
        ...
        # Limpa possíveis Z ou offsets manuais de fuso
        ...
        return f"Evento '{titulo}' agendado com sucesso no Google Agenda! Link: {link}"
    except Exception as e:
...
class AgenteSky:
    ...
    def responder(
        ...
        instrucao_sistema = {
            "role": "system",
            "content": (
                f"Você é a SKY, uma assistente virtual capaz de controlar o computador, realizar buscas na web e gerenciar compromissos.\n"
                f"Usuário: {nome_usuario} | Data/Hora Atual: {momento_atual}\n\n"
                f"FATOS CONHECIDOS SOBRE O USUÁRIO:\n{resumo_fatos}\n\n"
                f"REGRAS:\n"
                f"1. Use as ferramentas disponibilizadas quando necessário (abrir apps, agendar compromissos, pesquisar na web ou rodar comandos do sistema).\n"
                f"2. Se for agendar no calendário, converta datas relativas (ex: 'amanhã às 15h') em datas ISO reais 'YYYY-MM-DDTHH:MM:SS' usando a Data/Hora Atual como base.\n"
                f"3. Responda com clareza, objetividade e cordialidade."
            ),
        }
        ...
        try:
            ...
            # Executa ferramentas se o modelo solicitou
            if resposta.get('message', {}).get('tool_calls'):
                for tool in resposta['message']['tool_calls']:
                    ...
                    elif funcao_nome == 'pesquisar_na_web':
                        res_ferramenta = pesquisar_na_web(**argumentos)
                    elif funcao_nome == 'executar_comando_cmd':
                        res_ferramenta = executar_comando_cmd(**argumentos)
                    else:
            ...
            if "connection refused" in err_str.lower():
                return "❌ Erro: O serviço Ollama não está rodando. Abra o Ollama e tente novamente."
            elif "not found" in err_str.lower():
                return f"❌ Erro: O modelo '{self.modelo}' não foi baixado no Ollama. Execute 'ollama run {self.modelo}' no terminal."
...
class AppSky:
    ...
    def criar_tela_login(self):
        ...
        tk.Label(
            self.frame_login, text="Usuário:", fg=self.COR_TEXTO_PADRAO,
        ...
        tk.Label(
            self.frame_login, text="Senha:", fg=self.COR_TEXTO_PADRAO,
            bg=self.COR_FUNDO_JANELA, font=("Helvetica", 10, "bold")
        ).pack(anchor="w", pady=(10, 2))
        ...
        btn_entrar = tk.Button(
            self.frame_login, text="Entrar / Cadastrar",
    ...
    def criar_nova_aba_chat(self, sessao_id: str = None, titulo_aba: str = None):
        ...
        if conversas_salvas:
            ...
            self.adicionar_texto_chat(
                area_chat,
                f"✨ SKY: Olá, {self.nome_usuario}! Posso abrir programas no seu PC, realizar pesquisas na web, rodar comandos e agendar reuniões no seu Google Calendar. Como posso ajudar hoje?\n\n",
                "sky",
            )
