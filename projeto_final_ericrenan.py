"""
===============================================================================
PROJETO FINAL: IA "SKY" com Modelo Avançado e Memória Ativa
===============================================================================
Descrição:
    Este programa usa o Ollama para operar com o modelo 'llama3.1:8b',
    mantém uma memória resumida por usuário em SQLite e permite autenticação
    com cadastro e login seguros por hash SHA-256.
===============================================================================
"""
...
def atualizar_resumo_memoria(usuario_id: int, novo_resumo: str):
    ...
    cursor = conexao.cursor()
    cursor.execute(
        "UPDATE memoria_resumida SET resumo = ? WHERE usuario_id = ?;",
        (novo_resumo, usuario_id),
    )
    conexao.commit()
    conexao.close()
